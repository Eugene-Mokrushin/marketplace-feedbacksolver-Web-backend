  ...
  async generateFeedbackReply(
    ...
    try {
      ...
      const prompt = new PromptTemplate({
        ...
        template: `Ты представитель бренда {brand}, один из многих продавцов, который продает продукты на маркетплейсе. Твоя задача ответить на отзыв.  Деньги назад ты вернуть не можeшь.  Будь менее официален но обращайся на Вы, при обращении не пиши слово "привет", уложись масимум в 40-60 слов. Не упоминай про маркетплейс и название купленного продукта!  На товар {product_name}  поступил отзыв, с оценкой {score} из 5. Отзыв: {feedback}. ${
          isPersonalized && feedbackParams.buyer_name
            ? 'Обратись к покупателю по имени {buyer_name}. '
            : ''
        } ${
